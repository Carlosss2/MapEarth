package com.carlos.mapearth.features.home.domain.usescases

import com.carlos.mapearth.features.home.domain.entities.Country
import com.carlos.mapearth.features.home.domain.repositories.CountryRepository

class GetCountriesByCapitalUseCase(
    private val repository: CountryRepository
) {

    suspend operator fun invoke(capital: String): Result<List<Country>> {
        return try {

            if (capital.isBlank()) {
                return Result.failure(Exception("El nombre de la capital no puede estar vacío"))
            }

            val countries = repository.getCountriesByCapital(capital)

            val filteredCountries = countries.filter { it.name.isNotBlank() }

            if (filteredCountries.isEmpty()) {
                Result.failure(Exception("No se encontraron países con esa capital"))
            } else {
                Result.success(filteredCountries)
            }

        } catch (e: Exception) {
            Result.failure(e)
        }
    }
}